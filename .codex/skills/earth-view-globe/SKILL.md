...
4. `src/components/Globe/CameraHotkeys.tsx` for the floating imagery panel, number keys, overlay stack UI, and activity toggles.
...
- `src/components/Globe/Globe.tsx`: canvas, lights, starfield, Earth texture URLs, overlay textures, activity overlays, `AdaptiveControls`.
...
- `src/components/Globe/CameraHotkeys.tsx`: imagery panel, number-key layer switching, visibility toggle, overlay add/reorder/remove, activity toggles.
- `src/components/Globe/BoundaryLines.tsx`: Natural Earth boundaries and graticule geometry.
- `src/components/Globe/CityLabels.tsx`: tiered globe labels hidden when modal or max-zoom overlay is active.
- `src/components/Globe/EventOverlays/*`: USGS/EONET fetches and markers/tracks.
...
For labels/boundaries, keep `modalOpen` and `atMaxZoom` visibility behavior in mind. City labels intentionally disappear when the modal or max-zoom overlay is active.
